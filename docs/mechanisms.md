...
实现 `task.StoreInterface` 即可注入自定义后端（数据库、Redis 等）：

```go
app := goagent.New(
    goagent.WithTaskTools(),
    goagent.WithTaskStore(myDBTaskStore{}),
)
```

同样，Plan 系统实现 `plan.StoreInterface` 后通过 `WithPlanStore()` 注入。
...
默认不启用，需通过 `WithTaskTools()` Option 开启：

```go
app := goagent.New(goagent.WithTaskTools())
```
...
默认不启用，需通过 `WithPlanTools()` Option 开启：

```go
app := goagent.New(goagent.WithPlanTools())
```
...
app := goagent.New(
    ...
    goagent.WithPlanStore(myDBPlanStore{}),
)
```
...
| `MessageType` | any | string | message 队列元素零值 |
| `ResultType` | any | string | result 队列元素零值 |
