...
func TestApplyNodeDefaults(t *testing.T) {
	...
	if node.MessageType != reflect.TypeOf("") {
		t.Errorf("expected MessageType=string, got %v", node.MessageType)
	}
	if node.ResultType != reflect.TypeOf("") {
		t.Errorf("expected ResultType=string, got %v", node.ResultType)
	}
}
